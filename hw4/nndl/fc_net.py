...
class TwoLayerNet(object):
  ...
  """
    ...
    """
        ...
        dout1, grads['W2'], grads['b2'] = affine_backward(dout2, cache3)
        dout0 = relu_backward(dout1, cache2)
        dx, grads['W1'], grads['b1'] = affine_backward(dout0, cache1)
    ...
    """
        ...
        for i in range(self.num_layers):
            if i < self.num_layers - 1:
                if self.use_batchnorm:
                    out[i + 1], cache[i + 1] = affine_bn_relu_forward(out[i], self.params['W' + str(i + 1)],
                                                                      self.params['b' + str(i + 1)],
                                                                      self.params['gamma' + str(i + 1)],
                                                                      self.params['beta' + str(i + 1)],
                                                                      self.bn_params[i])
                else:
                    out[i + 1], cache[i + 1] = affine_relu_forward(out[i], self.params['W' + str(i + 1)],
                                                                   self.params['b' + str(i + 1)])

                if self.use_dropout:
                    out[i + 1], cache['dropout' + str(i + 1)] = dropout_forward(out[i + 1], self.dropout_param)

            else:
                out[i + 1], cache[i + 1] = affine_forward(out[i], self.params['W' + str(i + 1)],
                                                          self.params['b' + str(i + 1)])
