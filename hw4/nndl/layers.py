...
def batchnorm_backward(dout, cache):
  ...
  """
    ...
    N = x.shape[0]
    dx_norm = dout * gamma
    dx_var = np.sum(-0.5 * dx_norm * (x - sample_mean) / (sample_var + eps) ** 1.5, axis=0)
    dx_mean = -1 / np.sqrt(sample_var + eps) * np.sum(dx_norm, axis=0) - 2 * dx_var * np.mean(x - sample_mean, axis=0)
    dx = dx_norm / np.sqrt(sample_var + eps) + dx_mean / N + dx_var * 2 * (x - sample_mean) / N
