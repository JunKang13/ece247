...
def conv_forward_naive(x, w, b, conv_param):
  ...
  """
    ...
    for n in range(N):
        for f in range(F):
            for h1 in range(H_prime):
                for w1 in range(W_prime):
                    out[n, f, h1, w1] = np.sum(
                        x_padded[n, :, h1 * stride:h1 * stride + HH, w1 * stride:w1 * stride + WW] * w[f, :, :, :]) + b[
                                            f]
...
def conv_backward_naive(dout, cache):
  ...
  """
    ...
    for i in range(N):
        for j in range(F):
            for h1 in range(out_height):
                for w1 in range(out_width):
                    dw[j, :, :, :] += xpad[i, :, h1 * stride:h1 * stride + HH, w1 * stride:w1 * stride + WW] * dout[
                        i, j, h1, w1]
                    dx[i, :, h1 * stride:h1 * stride + HH, w1 * stride:w1 * stride + WW] += w[j, :, :, :] * dout[
                        i, j, h1, w1]
